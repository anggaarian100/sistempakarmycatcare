package com.example.mycatcare.Bayes;
...
public class Bayes {
    ...
    public void addNewLabel(double c[], double X[], int k){
        ...
        for(int i = 0; i<c.length; i++){
            ...
            Intent intent = new Intent("proses3");intent.putExtra("xcici", "P(X| "+ atribut.get(atribut.size()-1)+" = "+key.get(i).getInfo()+") * P("+ atribut.get(atribut.size()-1)+" = "+key.get(i).getInfo()+") = "+df.format(c[i]*X[i])+"\n\n");
            boolean b = LocalBroadcastManager.getInstance(context).sendBroadcast(intent);
        }
    }
